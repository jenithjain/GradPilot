#!/usr/bin/env python3
"""Writes the theme-aware AICounsellingDashboard.jsx"""
import os
...
export default function AICounsellingDashboard({ onClose, avatar, data }) {
  ...
  useEffect(() => {
    ...
    tl.to(aiCardRef.current, { boxShadow: T.aiGlowHi, duration: 2, ease: "power2.inOut" })
      .to(aiCardRef.current, { boxShadow: T.aiGlowLo, duration: 2, ease: "power2.inOut" });
    return () => tl.kill();
  }, [mounted, isDark]);
...
target = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "components", "AICounsellingDashboard.jsx")
